...
class Teachingscheduletasks extends CI_Controller {
    ...
    function __construct(){
        ...
        if(!getAdminViewer()->id){
            ...
            if(!in_array("/teachingscheduletasks/",$permission_urls)){
                redirect('error');
            }
        }
    }
    ...
    public function getTeachingscheduleTaskInfo($teachingscheduletask_id=0){
        $data['teachingscheduleTaks'] = $this->teachingscheduletask->getTeachingScheduleTask($teachingscheduletask_id);
        $data['filelist'] = $this->materialresource->getResourceInfoListByIds($data['teachingscheduleTaks']->material_resource_ids);
        $this->load->view('wePlatForm/tmpl/TeachingScheduleTaskEdit',$data);
    }
