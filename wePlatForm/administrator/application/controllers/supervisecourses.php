...
class Supervisecourses extends CI_Controller {
    ...
    function __construct(){
        ...
        if(!getAdminViewer()->id){
            ...
            if(!in_array("/supervisecourses/",$permission_urls)){
                redirect('error');
            }
        }
    }
    ...
    public function index($region_education_plan_id=0){
        ...
        $this->load->view('wePlatForm/supervisecoursesList',$data);
    ...
    public function add($region_education_plan_id =0){
        $data['region_education_plan'] = $this->regioneducationplan->getRegionEducationPlan($region_education_plan_id);
        $data['region_education_plan_detail'] = $this->region->getRegionEducationPlanInfo($data['region_education_plan']->plan_detail_id);
        $data['region'] = $this->region->getRegion( $data['region_education_plan_detail']->region_id);
        $data['schools'] = $this->school->getSchoolList($data['region_education_plan_detail']->region_id,9999,0);
