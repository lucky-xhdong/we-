...
<div class="modal fade new-icon-box" role="dialog">
  <div class="modal-dialog">
    <?php echo form_open('courses/updateLesson', 'class="form login-form" id="formContent" enctype="multipart/form-data"'); ?>
...
<script type="text/javascript" src="<?=base_url()?>media/administrator/js/lesson.js"></script>
<script>
  var LessonItem = {};
  LessonItem.id = 0;
  LessonItem.unit_id = <?=$unit->id?>;
  $(function() {
    $("#formContent").validate();
    $('.new-icon-box').on('shown.bs.modal', function() {
      $.ajax({
        type: "GET",
        url: '<?=anchorurl('courses/getLessonDetail')?>/'+LessonItem.id+'/'+LessonItem.unit_id,
        beforeSend:function(){
