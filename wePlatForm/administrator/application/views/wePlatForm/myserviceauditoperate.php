        ...
        <div class="col-md-9 fm--main-wrapper">
            ...
            <nav class="common-breadcrumb" aria-label="breadcrumb">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active" aria-current="page">运营计划</li>
                </ol>
            </nav>
            <!--面包屑 end-->
            <!--主体内容 start-->
            <div class="fm--main__content" data-before="运营计划">
                <!--选项卡标签 start-->
...
<!--                                </a>-->
<!--                            </div>-->
                            <div id="paginator_region_operate_plan">
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">运营计划拒绝理由</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <form action="<?=anchorurl('serviceaudit/addRegionOperatePlan/')?>" id="plan_form" class="form-group needs-validation" novalidate method="post">
    ...
    <div class="modal fade commonModal" id="region_operate_plan_modal" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                ...
                <form action="<?=anchorurl('regions/addRegionOperatePlan/0')?>" id="region_operate_plan_form" class="form-group needs-validation" novalidate method="post">
    ...
    <script>
        ...
        $(document).ready(function () {
            getOperatePlanmmgDocument(<?=getAdminViewer()->id?>);
            ...
            $(".status").click(function(){
                if($(this).is(":checked")){
                    mmgOperateDocument.load({status:$(this).val()});
                }
            });
            ...
            $('#region_operate_plan_modal').on('shown.bs.modal', function () {
                ...
                $.ajax({
                    type: "GET",
                    url: '<?=anchorurl('regions/getRegionOperatePlanInfo')?>/'+service_plan_id,
                    beforeSend:function(){
            ...
            $("#verifyRegionOperatePlan").on("click", function(){
                if( $("#region_operate_plan_form").valid()){
                    $("#verifyRegionOperatePlan").showLoading();
                    $("#region_operate_plan_form").ajaxForm({
                        ...
                        success: function (data) {
                            console.log(data);
                            $("#verifyRegionOperatePlan").hideLoading();
