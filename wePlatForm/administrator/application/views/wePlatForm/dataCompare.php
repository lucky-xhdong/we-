        ...
        <div class="col-md-9 ls--main-wrapper">
            ...
            <nav class="common-breadcrumb" aria-label="breadcrumb">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active" aria-current="page">数据对比</li>
                </ol>
            </nav>
            <!--面包屑 end-->
            <!--主体内容 start-->
...
<!--                        </select>-->
                ...
                <div class="learn-behavior" data-before="学习行为数据对比">
                    <div class="chart-wrapper">
                        <div class="form-row">
                            <div class="col-4 form-row__item" data-before="选择维度：">
                                <select  id="study_action" class="form-control form-control-sm">
                                    <option value="0">--选择数据查询维度--</option>
                                    <option value="1">登录人数对比</option>
                                    <option value="2">语言技能对比</option>
                                    <option value="3">学习计划达标率</option>
                                </select>
                            </div>
                            <div class="col-6 form-row__item" data-before="选择时间：">
                                <input type="text" id="startTime" class="datepicker form-control form-control-sm" placeholder="开始时间">
                                至
                                <input type="text" id="endTime" class="datepicker form-control form-control-sm" placeholder="结束时间">
                            </div>
                            <div class="col-1">
                                <a href="javascript:;" class="btn btn-outline-primary btn-sm btn-oprate" id="searchButton1">搜索</a>
                            </div>
                        </div>
                        <div id="main" style="width: 100%;height:400px;"></div>
                    </div>
                </div>
                <div class="learn-result" data-before="学习结果数据对比">
                    <div class="chart-wrapper">
                        <div class="form-row">
                            <div class="col-4 form-row__item" data-before="选择维度：">
                                <select  id="study_result" class="form-control form-control-sm">
                                    <option value="0">--学业等级对比--</option>
                                    <option value="1">优秀率对比</option>
                                    <option value="2">平均分对比</option>
                                    <option value="3">及格率对比</option>
                                </select>
                            </div>
                            <div class="col-6 form-row__item" data-before="选择时间：">
                                <input type="text" id="startTime1" class="datepicker form-control form-control-sm" placeholder="开始时间">
                                至
                                <input type="text" id="endTime2" class="datepicker form-control form-control-sm" placeholder="结束时间">
                            </div>
                            <div class="col-1">
                                <a href="javascript:;" class="btn btn-outline-primary btn-sm btn-oprate" id="searchButton2">搜索</a>
                            </div>
                        </div>
...
//                });
        ...
        function gettotalScoreNumOfUser(grade_id){
            ...
            myChart1.clear();
            var study_action = $("#study_action").val();
            var startTime = $("#startTime").val();
            var endTime = $("#endTime").val();
            $.get('<?=anchorurl("analysis/getdataComparetotalLoginUserOfUser/")?>/'+grade_id,{study_action:study_action,startTime:startTime,endTime:endTime,school_id: $("#school_id").val(),region_id:$("#region_id").val()}).done(function (data) {
                console.log(data);
        ...
        function getColligationScoreNumOfUser(grade_id){
            ...
            var study_result = $("#study_result").val();
            var startTime1 = $("#startTime1").val();
            var endTime1 = $("#endTime1").val();
            if(study_result ==2){
                $.get('<?=anchorurl("analysis/getdataComparetotalOutstandingUserOfUser/")?>/'+grade_id,{study_action:study_result,startTime:startTime1,endTime:endTime1,school_id: $("#school_id").val(),region_id:$("#region_id").val()}).done(function (data) {
                    data = JSON.parse(data);
                    myChart2.setOption({
                        tooltip : {
                            trigger: 'axis',
        ...
        gettotalScoreNumOfUser(0);
        getColligationScoreNumOfUser(0);
        $("#study_action").change(function(){
            gettotalScoreNumOfUser(grade_id);
        });
        $("#study_result").change(function(){
            getColligationScoreNumOfUser(grade_id);
        });
