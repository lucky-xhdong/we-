...
function getEducationPlanmmgDocument(){
    mmgEducationPlanDocument = $('#region_education_plan').mmGrid({
        ...
        url: base_url+'/serviceaudit/getRegionEducationPlanList/'+region_id,
        method: 'get',
        params:{
        ...
        if($(e.target).is('.btn-info, .btnPrice')){
            ...
            e.stopPropagation(); //阻止事件冒泡
            userItem = item;
            window.location.href = base_url+'/serviceaudit/educationPlan/'+item.id
            ...
            $.ajax({
                type: "POST",
                async:false,
                url: base_url+'/serviceaudit/addRegionEducationPlan',
                data: {
                    id:item.id,
