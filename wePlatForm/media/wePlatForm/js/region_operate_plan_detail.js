...
function getOperatePlanmmgDocument(user_id){
    mmgOperateDocument = $('#region_operate_plan').mmGrid({
        ...
        url: base_url+'/regions/getRegionOperatePlanList/'+region_id,
        method: 'get',
        params:{
            user_id:user_id
        },
        ...
        if($(e.target).is('.btn-info, .btnPrice')){
            ...
            $.ajax({
                type: "POST",
                async:false,
                url: base_url+'/regions/addRegionOperatePlan',
                data: {
                    id:item.id,
                    status:item.status==1?0:1,
                },
            ...
            console.log($(e.target));
            e.stopPropagation(); //阻止事件冒泡
            window.location.href = base_url+'/regions/operatePlan/'+item.id;
        }
