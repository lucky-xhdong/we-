        ...
        renderer: function (val,item) {
            if(item.verify == true){
                if(item.status == 1){
                    ...
                    return '<a href="javascript:;" class="btn-normal btn-edit  edit">编辑</a> <a href="javascript:;" class="btn-normal btn-view view">日程安排</a>';
...
function getEducationPlanmmgDocument(user_id){
    mmgEducationPlanDocument = $('#region_education_plan').mmGrid({
        ...
        url: base_url+'/regions/getRegionEducationPlanList/'+region_id,
        method: 'get',
        params:{
            user_id:user_id
        },
        ...
        if($(e.target).is('.btn-info, .btnPrice')){
            ...
            e.stopPropagation(); //阻止事件冒泡
            userItem = item;
            window.location.href = base_url+'/regions/educationPlan/'+item.id
