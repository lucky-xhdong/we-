...
//			printf($e->getMessage() . "\n");
    ...
    public function putObjectAcl( $object)
    {
        $bucket = Common::getBucketName();
        $ossClient = Common::getOssClient();
        $result = $ossClient->putObjectAcl($bucket, $object,   $ossClient::OSS_ACL_TYPE_PUBLIC_READ);
    }


    public function createSignUrlWeSpeakStyle($object){
        $timeout = 3600;
        $signedUrl = "";
        try {
            $ossClient = Common::getOssClient();
            $bucket = Common::getBucketName();
            // 生成GetObject的签名URL。
            $signedUrl = $ossClient->signUrl($bucket, $object, $timeout,'GET',array(
                OssClient::OSS_PROCESS => "style/wespeak_avatar",
            ));
        } catch (OssException $e) {
//			printf(__FUNCTION__ . ": FAILED\n");
//			printf($e->getMessage() . "\n");
