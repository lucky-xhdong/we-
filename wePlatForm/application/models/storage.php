...
class Storage extends CI_Model
{
    ...
    public $sizes = array(
        ...
        'article'     => array('width'=>300,'height'=>140),
        'article_big'  => array('width'=>780,'height'=>452),
        'article_large'  => array('width'=>380,'height'=>216),
        'article_large_1'  => array('width'=>580,'height'=>360),
        'article_large_2'  => array('width'=>380,'height'=>240),
    );
    ...
    public function uploadContentFile(){
            $realfiepath = UPLOADFILEPATH.'content/images';
   ...
   *
   * */
    public function uploadDocMediaFile(){
        if($this->_minxer){
            $realfiepath = UPLOADFILEPATH.$this->_minxer->objectType.'/n'.$this->_minxer->id;
            if (!is_dir($realfiepath))
            {
            ...
            $config['upload_path'] = $realfiepath;
            $config['allowed_types'] = 'docx|doc|pptx|ppt|jpeg|jpg|png|pdf|zip|mp3|xls|xlsx|mp4';
            $config['max_size'] = 8*1024*1024;
            $this->load->library('upload', $config);
            //$this->upload->set_allowed_types('mp3|wav|caf');
            if(!$this->upload->do_upload('file')){
