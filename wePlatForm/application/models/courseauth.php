...
class Courseauth extends CI_Model{
    ...
    public function syncStudentCourseData($user,$courser_id){
        ...
        foreach($units as $unit){
            $unit_relation_num = $this->db
                    ->where("course_id",$courser_id)
                    ->where("unit_id",$unit->id)
                    ->where("user_id",$user->id)
                    ->get('user_relation_course_unit')->result();

            if(count($unit_relation_num) == 0){
            ...
            }else if(count($unit_relation_num) > 1){
                foreach($unit_relation_num as $key=> $item){
                    if($key == 0){
                        $this->db->where("id",$item->id)->delete("user_relation_course_unit");
                    }
                }
            }
            ...
            foreach($lessons as $lesson){
                ...
                if(count($lesson_relation_num) == 0){
                    $relation_course_unit = array(
                        "lesson_id" => $lesson->id,
                        "unit_id"   => $unit->id,
                        "user_id"   => $user->id,
                ...
                }else if(count($lesson_relation_num) > 1){
                    foreach($lesson_relation_num as $key1=> $item1){
                        if($key1 == 0){
                            $this->db->where("id",$item1->id)->delete("user_relation_unit_lesson");
                        }
                    }
                }
            }
