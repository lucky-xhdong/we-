...
class Users extends CI_Controller {
    ...
    public function login(){
        ...
        if(isset($post['type']) && $post['type'] == "guanlin"){
            return  $this->authenticationwetalk->onAuthenticateThird($post);
        }
    ...
    public function loginThird(){
        $post = $this->input->post();
        $post['type'] = "guanlin";
        $post['uuid'] = "0FE400AA-7885-450A-9C02-1DEE7BF5C927";
        $post['password'] = "3018397f4f23d51e947208be1734e48f";

        $data =   $this->authenticationwetalk->onAuthenticateThird($post);
        var_dump($data);
...
//     }
    ...
    public function getMyRadarWebView($user_id){
        ...
        $this->load->view('studentdatachartnews',$data);
    }


    public function getMyRadarWebViewNew($user_id){
        $data['user'] = $this->user->getUser($user_id);
        $data['userRecordInfo'] = $data['user']->getUserRecordDataInfo();
...
//        return $this;
    ...
    public function register(){
        ...
        if($app_version && $app_version != "1.2.3") {
...
//        $data['email'] = "wuxi@163.com";
    ...
    public function getStudentRankingListJson($class_id=0){
        echo json_encode($this->classes->getClassUsersInfoPc($class_id));
        exit;
    }
    ...
    public function getMyStudentData($user_id){
        $data['user'] = $this->user->getUser($user_id);
        $data['class_id'] = 0;
        $this->load->view('studentData',$data);
    }

    public function getMyClassData($class_id){
        $data['user'] = false;
        $data['class_id'] = $class_id;
    ...
    public function getParticipationDegreeInfoList($user_id){
        if($user_id !=0 ){
            $user = $this->user->getUser($user_id);
            if($user){
                $post = $this->input->post();
                if(!isset($post['start_date']) || ! isset($post['end_date'])){
                    ...
                    echo json_encode($this->recordmanager->getMyParticipationDegreeTime($class_id)->returncode);
                }
            }else{
                echo json_encode($this->returncode);
            }
