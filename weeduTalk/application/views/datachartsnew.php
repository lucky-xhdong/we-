...
<head>
    ...
    <meta charset = "utf-8">
    <title>班级学习数据--<?=$classItem->name?></title>
...
<div>
    ...
    <ul class="study-list">
        <li>
            <span>总学习时间</span>
            <font><?=$classItem->study_num_time?><em>H</em></font>
        </li>
        <li>
            <span>总学习天数</span>
            <font><?=$classItem->study_num_day?><em>天</em></font>
        </li>
        <li>
            <span>本周学习时间</span>
            <?php
            $usercount = $classItem->geClassUserScoreCount($classItem->id);
            ?>
            <font>
                <?php if($usercount != 0):?>
                    <?=round($classItem->study_num_week_time/$usercount,2)?>
                <?php else:?>
                    <?=$classItem->study_num_week_time?>
                <?php endif;?>
                <em>H</em></font>
        </li>
    </ul>
...
<div class="portrait-progressbar">
    <p class="txt-title" data-before="班级学情概览(月)">
<!--        --><?php
//        $day =  date("d");
//        ?>
<!---->
    ...
    $(function(){
        var chart = new Highcharts.Chart('container', {
            ...
            xAxis: {
                ...
                categories:["<?=implode('","',array_keys($chatClassLine))?>"],
                labels: {
                    step:1,
                    style: {
                        color: '#bbc9ec'
            ...
            series: [{
                name: '人数',
                data: [<?=implode(',',array_values($chatClassLine))?>],
                marker:{//线上数据点
                    radius:4,
                    lineWidth:2,
                    lineColor:'#8a9ba2',
