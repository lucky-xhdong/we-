...
<div class="evaluation-container">
    ...
    <!--header end-->
    <!--基本信息 start-->
    <div class="baseinfo" data-before="<?=$exam->name?>">
        <div class="baseinfo__left">
            <p class="txt" data-before="姓&emsp;&emsp;名："><?=$user->name?></p>
            <p class="txt" data-before="学&emsp;&emsp;校："><?=$schoolName?></p>
<!--            <p class="txt" data-before="测评组别：">小学组</p>-->
        </div>
        <div class="baseinfo__right">
            <span class="total-score" data-before="成绩："><?=$result->totalScores?>分</span>
            <span class="txt-time">测试时间:<?=date("Y年m月d日H:i",strtotime($exam->exam_time))?>-<?=date("H:i",strtotime($result->createdTime))?></span>
        </div>
    </div>
    <!--基本信息 end-->
    <!--综合分析 start-->
    <div class="comprehensive-analysis" data-before="综合分析">
        <div class="comment">
        ...
        <div class="chart-info">
            <?php foreach($firstComments as $firstComment):?>
                <p class="txt">您在<?=$firstComment->name?>模块测试中得分率为<?=$firstComment->ratings*100?>%，
    ...
    <!--综合分析 end-->
    <!--测评成绩分析与评价 start-->
    <div class="achievement-analysis" data-before="测评成绩分析与评价">
        <div class="analysis-table-wrapper" data-before="<?=$user->name?>老师，根据您本次考试的答题情况，我们得出以下分析结果：">
...
<!--                </tr>-->
    ...
    <!--测评成绩分析与评价 end-->
    <!--培训建议 start-->
    <div class="training-suggestions" data-before="培训建议">
