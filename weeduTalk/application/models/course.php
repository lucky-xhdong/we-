...
class Course extends CI_Model{
    ...
    public function _initialize($config=array())
    {
        $config['attributes'] = array(
            ...
            "filename_v2"            => array("require" => false),

            "status"              => array("require" => false),
            "nickname"              => array("require" => false),
    ...
    public function getCourseInfo($user=null,$publishing_house_courses_id=0){
        $get = $this->input->get();
        $data = array(
            'id'                =>(int)$this->id,
            'name'              => $this->name,
            ...
            "picture_v2_url"    => $this->getV2FileUrl(),
            'status'            =>  $this->status,
        ...
        if($user && $publishing_house_courses_id != 0 &&  isset($get['app_version']) && $get['app_version'] >= "2.0.0"){
            $this->recordmanager->initialize()->user_id = $user->id;
            $data['last_study_record'] =  $this->recordmanager->getLastStudyPart($user,$publishing_house_courses_id);
            $authunitC = $this->courseauth->getUserCourseLessonRowAVG($user,(int)$this->id);
            if($authunitC){
                $data['completion_rate']=round($authunitC->completion_rate,2);
                $data['accuracy']  =round($authunitC->accuracy,2);
            }
            if(!empty($this->filename_v2)){
                $data['picture_url'] = $this->getV2FileUrl();
            }
        }
        return $data;
    }
    ...
    public function getV2FileUrl(){
        if(!empty($this->filename_v2)){
            $picture = $this->storage->setMinxer($this)->getFileName($this->filename_v2,"origin");
            return $picture;
        }
        return base_url().'media/assets/courses/'.$this->filename_v2;
    }
    ...
    public function getCourseList($user=null){
        ...
        $courses = $courses->order_by("sort","asc")->get("courses")->result();

        $result = array();
        foreach($courses as $course){
            $result[] = $this->getCourse($course->id)->getCourseInfo($user,$course->id);
        }
    ...
    public function getAdminCourseList($user=null){
        ...
        if($user != null){
            ...
            if(count($coursearray) > 0){
                $courses = $this->db->select('id')->where_in("id",$coursearray)->order_by("sort","asc")->get("courses")->result();
                foreach($courses as $course){
    ...
    public function getLevelList($course_id=2,$user=null){
        $get = $this->input->get();
        ...
        if($user && $user->id == 53){
            foreach($levels as $level){
                ...
                $level->accuracy      =0.3;
                if($user  &&  isset($get['app_version']) && $get['app_version'] >= "2.0.0"){
                    $units = $this->db->select('id')->where("level_id",$level->id)
    ...
    public function getAdminCoursesDetails($user=null){
        $data = $this->config['attributes'];
        if($user != null && !empty($user->course_ids) && $user->id != 53){
            $coursearray = explode(",",$user->course_ids);
            $courses = $this->db->select(implode(',',array_keys($data)))->where_in("id",$coursearray);
        ...
        //$courses =  $this->db->select(implode(',',array_keys($data)))->get("courses")->result();
        foreach($courses as $key => $course){
            $levels = $this->db->select('id,name,status')->where("course_id",$course->id)->get("levels")->result();
            $course->levels = $levels;
            if(count($levels) >0){
                $units = $this->unit->getAdminUnitsInfo($course->id,$user,$levels[0]->id);
            }else{
                $units = $this->unit->getAdminUnitsInfo($course->id,$user,0);
            }
            $course->units = $units;
        }
        $this->returncode['data'] = $courses;
    ...
    public function getAdminPublishUnLockCoursesDetails($user=null,$class=null){
        $data = $this->config['attributes'];
        $school_id = 0;
        if($user){
        ...
        //$courses =  $this->db->select(implode(',',array_keys($data)))->get("courses")->result();
        foreach($courses as $key => $course){
            $units = $this->publish_hourse_course_unit->getpublichUnlockUnitsInfo($course->publishing_house_courses_id);
    ...
    public function getCourseLearningTimeDetails($user=null,$date=null){
        $school_id = 0;
        if($user){
            $UserSchoolGradeClassRealtionShip = $user->getUserSchoolGradeClassRealtionShip();
            $school_id = $UserSchoolGradeClassRealtionShip->school_id;
        ...
        if($user && $date){
            $this->recordmanager->initialize()->user_id = $user->id;
            $courseOBJ =  $this->recordmanager->getMyDateStudyTimeData($date);
            if($courseOBJ){
                if($user->id != 53){
                    $courses = $this->db->select("courses.*,publishing_house_courses.id as publishing_house_courses_id")
                        ->where_in("publishing_house_courses.course_id",$this->toIdArray($courseOBJ));
                    if($school->id !=0){
                        $courses = $courses->where("publishing_house_courses.publishing_house_id",$school->publishing_house_id);
                    }
                    $courses = $courses->join('courses','courses.id = publishing_house_courses.course_id', 'left')->where("courses.status",1);
                    $courses = $courses->order_by("publishing_house_courses.sort","asc")->get("publishing_house_courses")->result();
                }else{
                    $courses = $this->db->select("*")
                        ->where_in("id",$this->toIdArray($courseOBJ))
                        ->order_by("sort","asc")->get("courses")->result();
                }
            ...
            foreach($courses as $key => $course){
                if(empty($course->nickname))  $course->nickname = $course->name;
                if($user->id != 53){
                    $units = $this->publish_hourse_course_unit->getkUnitsLearningTimeInfo($course->publishing_house_courses_id,$user,$date);
                }else{
                    $units = $this->publish_hourse_course_unit->getkUnitsLearningTimeInfo($course->id,$user,$date);
                }
                $course->units = $units;
            }
        }
    ...
    public function getCoursesMtDetails($user=null){
        ...
        $courses = $courses->where("has_test",1)->order_by("sort","asc")->get("courses")->result();

        // $courses =  $this->db->select(implode(',',array_keys($data)))->order_by("sort","asc")->get("courses")->result();
        foreach($courses as $key => $course){
    ...
    public function getPublishCoursesMtDetailsClient($user=null,$class=null){
        $data = $this->config['attributes'];
        $school_id = 0;
        if($user){
            $UserSchoolGradeClassRealtionShip = $user->getUserSchoolGradeClassRealtionShip();
        ...
        $courses = $courses->join('courses','courses.id = publishing_house_courses.course_id', 'left')
            ->where("courses.status",1)->where("courses.has_test",1);
        ...
        //$courses =  $this->db->select(implode(',',array_keys($data)))->get("courses")->result();
        foreach($courses as $key => $course){
            $units = $this->publish_hourse_course_unit->getpublichUnitsMtInfo($course->publishing_house_courses_id,$user);
    ...
    public function getPublishCoursesMtDetails($user=null){
        $data = $this->config['attributes'];
        $school_id = 0;
        if($user){
            $UserSchoolGradeClassRealtionShip = $user->getUserSchoolGradeClassRealtionShip();
        ...
        if($user != null && !empty($user->course_ids)){
            $coursearray = explode(",",$user->course_ids);
            $courses = $this->db->select("courses.*,publishing_house_courses.id as publishing_house_courses_id")->where_in("publishing_house_courses.course_id",$coursearray);
        }
        if($school->id !=0){
        ...
        foreach($courses as $key => $course){
            ...
            $units = array();
            if(count($levels) >0){
                $units = $this->publish_hourse_course_unit->getUnitsMtInfo($course->publishing_house_courses_id,$user,$levels[0]->id);
            }
            $course->units = $units;
        }
        $this->returncode['data'] = $courses;
    ...
    public function getPublishCoursesMtForLevelDetails($user,$publishing_house_courses_id=0,$level_id=0){
        $units = $this->publish_hourse_course_unit->getUnitsMtInfo($publishing_house_courses_id,$user,$level_id);
        $this->returncode['data'] = $units;
        return $this;
    }
...
//            echo $this->db->last_query();
    ...
    public function uploadV2picture()
    {

        $file = $this->storage->setMinxer($this)->uploadBackGroundFile();
        if ($file->errorCode == 0) {
            $data['filename_v2'] = $file->data['file_name'];
            $this->updateKey($data);
            $this->returncode['data'] = $this->getFileUrl();
        } else {
