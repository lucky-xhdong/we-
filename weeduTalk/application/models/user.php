...
//            "calculation_date_time" => array("require" => false)
    ...
    public function getUser($id)
    {
        ...
        if ($user) {
            ...
            if((!$permissions || count($permissions) == 0)) {
             ...
             //   echo "22222".$iuser->setCourseIds()->course_ids;
            }
        } else {
            $iuser->id = 0;
        }
    ...
    public function setCourseIds(){
        ...
        $releationships =  $this->db->select("classes.course_ids,class_user_relationship.user_type")->where("class_user_relationship.user_id", $this->id)
...
//            ->where("classes.id is not null")
        ...
        $course_array = array();
        if($releationships){
            foreach ($releationships as $releationship){
                if($releationship->user_type == "teacher"){
                    $isteacher = true;
                }
                if(!empty($releationship->course_ids)){
                    $course_id_child_array = explode(",",$releationship->course_ids);
                    $course_array = array_merge($course_array,$course_id_child_array);
                }

            }
            if($isteacher){
                $item1->user_type =  "teacher";
            }else{
                $item1->user_type =  "student";
            }

            if(count($course_array) >  0){
                foreach ($course_array as $key => $course_arra){
                    if(empty($course_arra)){
                        unset($course_array[$key]);
                    }

                }
                $item1->course_ids = implode(",",$course_array);
            }else{
...
//            'mobile' => $this->mobile,
            ...
            "my_participation_degree_url"=> anchorurl('components/users/getMyStudentData/' . $this->id),
    ...
    public function getUserRecordDataInfoNew(){

        $row = $this->db->select("study_num_time,study_num_day,study_num_keys,calculation_date_time")->where("id",$this->id)->get("users")->row();
        if($row){
            return $row;
        }
        ...
        if(isset($studytime->totalTime) && !empty($studytime->totalTime))  $val->study_num_time = $studytime->totalTime;
        else $val->study_num_time = 0;
        if(isset($totalday->totalday) && !empty($totalday->totalday))  $val->study_num_day = $totalday->totalday;
        else $val->study_num_day = 0;
        if($records){
        ...
        $this->db->update("users",array(
            "study_num_time"=>$val->totalTime,
            "study_num_day"=>$val->totalday,
            "study_num_keys"=>$val->study_num_keys,
    ...
    public function getUserInfoDetail($user = null)
    {
        $data = array(
            'id' => (int)$this->id,
            'username' => $this->username,
        ...
        $detail = $this->getUserRecordDataInfoNew();
        $data['study_num_time'] = $detail->study_num_time;
        $data['study_num_day'] = $detail->study_num_day;
        $data['study_num_keys'] = $detail->study_num_keys;
...
//            'mobile' => $this->mobile,
  ...
  **/
    ...
    public function getUserSchoolGradeClassRealtionShipStatusxxxxx()
    {
    ...
    {
        $releationship =  $this->db->select("class_user_relationship.*")->where("class_user_relationship.user_id", $this->id)
            ->where("class_user_relationship.status",1)
            ->join('classes','classes.id = class_user_relationship.class_id', 'left')
            ->limit(1)
//            ->where("classes.id is not null")
