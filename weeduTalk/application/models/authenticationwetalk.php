            ...
            if($row){
                ...
                if ($crypt == $testcrypt){
                    $user = $this->user->getUser($row->id);
                     if(empty($user->course_ids) && isset($post['type']) && $post['type'] == "evaluation"){
                          // 可以过
                     }else if(!empty($user->course_ids)){
                         // 可以过
                     }else{
                         $this->returncode['errcode'] = 1000091;
                        $this->returncode['errdesc'] = "无使用权限!";
                         return $this;
                     }
                    //检查班级权限
                    ...
                    if($getUserSchoolGradeClassRealtionShip){
                        $school = $this->db->select("status")->where("id", $getUserSchoolGradeClassRealtionShip->school_id)->get("school")->row();
                        if($school->status == 1){
                            if($user->status == 3){
                            ...
                            $this->returncode['errcode'] = 999998;
                            $this->returncode['errdesc'] = "您账户已暂停使用!";
                        }
                    }else{
