...
,wetalk_class_user_relationship.user_id,wetalk_user_agent.agent ,wetalk_user_agent.date,wetalk_user_agent.version,wetalk_user_agent.platform,wetalk_users.name , wetalk_users.username  FROM wetalk_class_user_relationship  LEFT JOIN  wetalk_user_agent  on wetalk_class_user_relationship.user_id  = wetalk_user_agent.user_id LEFT JOIN wetalk_users on wetalk_class_user_relationship.user_id  = wetalk_users.id  LEFT JOIN wetalk_grades on wetalk_class_user_relationship.grade_id  = wetalk_grades.id  LEFT JOIN wetalk_classes
 on wetalk_class_user_relationship.class_id  = wetalk_classes.id  where wetalk_class_user_relationship.user_type = "student" and  wetalk_class_user_relationship.school_id  = '.$this->id.' and wetalk_user_agent.date  >="'.$start_date.'"  and wetalk_user_agent.get_method = "getOssInfo2"  group by wetalk_user_agent.user_id  ORDER BY  wetalk_user_agent.id desc ) p GROUP BY p.user_id ';
    ...
    public function importSchoolUserScoreDataList(){
        ...
        $result = $this->db->select("users.username,users.name ,school.name  as schoolName,grades.name  as gradeName,classes.name  as className")
            ->where("school.id",$this->id);
        if(isset($get['start_data']) && !empty($get['start_data'])){
            $result = $result->where("user_record_result.start_date >=",$get['start_data']);
        }
        if(isset($get['end_date']) && !empty($get['end_date'])){
            $result = $result->where("user_record_result.start_date <=",$get['end_date']);
        }

        if((isset($get['start_data']) && !empty($get['start_data'])) || (isset($get['end_date']) && !empty($get['end_date']))){
            $result = $result->group_by("user_record_result.user_id")->select("AVG(wetalk_user_record_result.studyReuslt) as studyReuslt,AVG(wetalk_user_record_result.StudyTimeAndFrequency) as StudyTimeAndFrequency,AVG(wetalk_user_record_result.studyCourseAverage) as studyCourseAverage,AVG(wetalk_user_record_result.SkillAverage) as SkillAverage,AVG(wetalk_user_record_result.StudyComprehensive) as StudyComprehensive,AVG(wetalk_user_record_result.totalScore) as totalScore");
        }else{
            $result = $result->select("user_record_result.*")
                ->where("user_record_result.start_date",$impportdte);
        }

        $result = $result->join('class_user_relationship','class_user_relationship.user_id = user_record_result.user_id', 'left')
            ->join('users','class_user_relationship.user_id = users.id', 'left')
            ->join('classes','user_record_result.class_id = classes.id', 'left')
            ->join('grades','user_record_result.grade_id = grades.id', 'left')
            ->join('school','user_record_result.school_id = school.id', 'left')
            ->join('region','school.region_id  = region.id', 'left')
            ->get("user_record_result")
            ->result();
...
//        $result = $this->db->query($query)->result();
        foreach($result as $key=>$val){
            if((isset($get['start_data']) && !empty($get['start_data'])) && (isset($get['end_date']) && !empty($get['end_date']))){
                $dateString = date("m月d日",strtotime($get['start_data']))." 至 ".date("m月d日",strtotime($get['end_date']));
            }else if(isset($get['start_data']) && !empty($get['start_data'])){
                $dateString = date("m月d日",strtotime($get['start_data']))." 至 ".date("m月d日",strtotime($impportdte));
            }else if(isset($get['end_date']) && !empty($get['end_date'])){
                $dateString = "09-01"." 至 ".date("m月d日",strtotime($get['end_date']));
            }else{
                $dateString = $impportdte;
            }
            $item= array(
                "序号"            => $key+1,
                "区域"            => $region->name,
                ...
                "用户名"           => $val->username,
                "日期"            =>  $dateString,
                "学习效果"         => round($val->studyReuslt,2),
                "时间与频率"       => round($val->StudyTimeAndFrequency,2),
                "学习进度"         => round($val->studyCourseAverage,2),
                "技能情况"         => round($val->SkillAverage,2),
                "学习方法"         =>round( $val->StudyComprehensive,2),
                "总分"            => round($val->totalScore,2)
            );
    ...
    public function importSchoolClassScoreDataList(){
        ...
        foreach($classes as $key=> $classe){
            ...
            $val = $this->db->select("grades.name  as gradeName")
                ->where("classes.id",$classe->id);
            if(isset($get['start_data']) && !empty($get['start_data'])){
                $val = $val->where("user_record_result.start_date >=",$get['start_data']);
            }
            if(isset($get['end_date']) && !empty($get['end_date'])){
                $val = $val->where("user_record_result.start_date <=",$get['end_date']);
            }

            if((isset($get['start_data']) && !empty($get['start_data'])) || (isset($get['end_date']) && !empty($get['end_date']))){
                $val = $val->select("AVG(wetalk_user_record_result.studyReuslt) as studyReuslt,AVG(wetalk_user_record_result.StudyTimeAndFrequency) as StudyTimeAndFrequency,AVG(wetalk_user_record_result.studyCourseAverage) as studyCourseAverage,AVG(wetalk_user_record_result.SkillAverage) as SkillAverage,AVG(wetalk_user_record_result.StudyComprehensive) as StudyComprehensive,AVG(wetalk_user_record_result.totalScore) as totalScore");
            }else{
                $val = $val->select("AVG(wetalk_user_record_result.studyReuslt) as studyReuslt,AVG(wetalk_user_record_result.StudyTimeAndFrequency) as StudyTimeAndFrequency,AVG(wetalk_user_record_result.studyCourseAverage) as studyCourseAverage,AVG(wetalk_user_record_result.SkillAverage) as SkillAverage,AVG(wetalk_user_record_result.StudyComprehensive) as StudyComprehensive,AVG(wetalk_user_record_result.totalScore) as totalScore")
                    ->where("user_record_result.start_date",$impportdte);
            }

            $val = $val->join('class_user_relationship','class_user_relationship.user_id = user_record_result.user_id', 'left')
                ->join('classes','user_record_result.class_id = classes.id', 'left')
                ->join('grades','user_record_result.grade_id = grades.id', 'left')
                ->get("user_record_result")
                ->row();
