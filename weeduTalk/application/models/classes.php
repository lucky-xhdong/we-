...
class Classes extends CI_Model{
    ...
    public function getStudentRankingList($userItem){
        ...
        if($userRelationShip){
            ...
            if(!isset($return['myRank'])){
                $user = $userItem->getUserInfoNotToken();
                $user['studyTime'] = 0;
                $user['totalScore'] = 0;
                $user['totalDay'] = 0;
                $user['totalDay']= 0;
...
//        if(isset($post['sort']) && $post['sort'] == "studyscore") {
//            $month_frist_day = date('Y-m-d', strtotime(' -1 day'));
//        }
...
//        }
    ...
    */
    ...
    public function getClassInfoList($grade_id=0,$limit=20,$start=0){
        ...
        $items = $items->get("classes")->result();
    ...
    public function getClassUserList($school_id=0,$grade_id=0,$class_id=0,$type="all",$limit=20,$start=0,$post){
        ...
        if(count($user_array_ids) > 0){
            $items = $this->db->select("id,nickname,name,user_type,username,avatar,course_ids,block,status")->where_in("id", $user_array_ids)->get("users")->result();
            foreach($items as $item){
                $item->avatar_url = $this->user->getAvatarUrlFormAvatar($item);
                $item->my_radar_charts_url = anchorurl('components/users/getMyRadarWebView/' . $item->id);
                $users[$item->id] = $item;
    ...
    */
    ...
    public function getLearningSituationOverviewForClass($class_id){
        ...
        $entity            =  $this->db->select("AVG(studyReuslt) as studyReuslt,AVG(studyCourseAverage) as studyCourseAverage,AVG(SkillAverage) as SkillAverage,AVG(StudyTimeAndFrequency) as StudyTimeAndFrequency,AVG(StudyComprehensive) as StudyComprehensive")
            ->where("class_id",$class_id)
            ->where("start_date",date("Y-m-d"))->get("user_record_result")->row(); //学习效果
        if(!$entity){
            $entity            =  $this->db->select("AVG(studyReuslt) as studyReuslt,AVG(studyCourseAverage) as studyCourseAverage,AVG(SkillAverage) as SkillAverage,AVG(StudyTimeAndFrequency) as StudyTimeAndFrequency,AVG(StudyComprehensive) as StudyComprehensive")
                ->where("class_id",$class_id)
                ->where("start_date", date('Y-m-d',strtotime( '- 1 days')))->get("user_record_result")->row(); //学习效果
        }
