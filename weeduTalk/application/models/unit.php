...
//            $data['scores'] = $this->recordmanager->getUserMTScores((int)$lesson->id);
            ...
            if($auhtUnit){
                ...
                $authunitC = $this->courseauth->getUserUnitLessonRowAVG($user,(int)$this->id);
                if($authunitC){
                    if($authunitC->completion_rate >= $auhtUnit->completion_rate){
                        $auhtUnit->completion_rate = $authunitC->completion_rate;
                    }

                    if($authunitC->accuracy >= $auhtUnit->accuracy){
                        $auhtUnit->accuracy = $authunitC->accuracy;
                    }
...
//                    if($authunitC->mt_score >= $auhtUnit->mt_score){
//                        $auhtUnit->mt_score = $authunitC->mt_score;
//                    }

                }

//                if(empty($auhtUnit->calculate_date) && empty($auhtUnit->accuracy) && empty($auhtUnit->completion_rate)  && $this->id < 123){
...
//                    ));
//                }elseif(empty($auhtUnit->calculate_date)  || ( !empty($auhtUnit->calculate_date) && ((time()-strtotime($auhtUnit->calculate_date)) > 10*60 ))) {
//                    $this->recordmanager->initialize()->user_id = $user->id;
//                    $this->recordmanager->unit_id = $this->id;
//                    $data['accuracy']        =   $this->recordmanager->getMyCorrection();
//
    ...
    public function calculateUnit($user= null){
        if($user) {
            ...
            if ($auhtUnit) {
                $this->recordmanager->initialize()->user_id = $user->id;
                $this->recordmanager->unit_id = $this->id;
                $data['accuracy'] = $this->recordmanager->getLearningTimeList();
                $this->recordmanager->initialize()->user_id = $user->id;
                $this->recordmanager->unit_id = $this->id;
                $data['completion_rate'] = $this->recordmanager->getMyCompletion();
                if ($data['accuracy'] >= 1) $data['accuracy'] = 1;
...
//            echo $this->db->last_query();
    ...
    public function getDigitalPortfolioList($unit_id = 0,$user,$limit=20,$start=0,$part_id=0){
        $data = array();
        $event_part_id = $this->db->select("part_id as id")
            ->where("unit_id",$unit_id)
            ->where_in("type",array("middle_school_new_writing","speech"))
            ->get("lessons_part_events")->result();

        $parts = $this->db->select('lessons.name as LessonName,lessons_part.name ,lessons_part.id')
            ->where("lessons_part.status",1)
            ->where_in("lessons_part.id",$this->toIdArray($event_part_id))
            ->join('lessons','lessons.id = lessons_part.lesson_id', 'left')
            ->get("lessons_part")->result();
        $category = array();
        foreach ($parts as $part){
            $category[] = array(
                "name"=>  $part->LessonName.'-'.$part->name,
                "id"  =>$part->id
            );
        }
        $data['parts'] = $category;
        $data['records'] = $this->getSpeechContentList($user,$limit,$start,$part_id);
        $this->returncode['data'] = $data;
        return $this;

    }

    public function getSpeechContentList($user,$limit=20,$start=0,$part_id = 0){
        $records = array();

        //获取所有是event的part_id
        $recordItems = $this->db
            ->select("lessons_part.name as partName,users_record_speech.*")
            ->where("users_record_speech.filename is not null")
            ->join('lessons_part', 'lessons_part.id = users_record_speech.part_id', 'left')
            ->where("users_record_speech.user_id",$user->id)
            ->order_by("users_record_speech.start_time","desc");
        if($part_id != 0){
            $recordItems = $recordItems->where("users_record_speech.part_id",$part_id);
        }
    ...
    public function getRecordItem($record){
        ...
        if(isset($record->id) && $record->id != 0 ){
            ...
            $record->name = $record->partName;
            $record->objectType = $record->type;
