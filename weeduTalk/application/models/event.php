...
class Event extends CI_Model{
    ...
    public function _initialize($config=array())
    {
        $config['attributes'] = array(
            ...
            "category_first_id"   => array("require" => false),
            "category_second_id"   => array("require" => false)
    ...
    public function getJsonPartEventList($part_id=0,$part,$lesson){
        $evnets = $this->db->select('id')  ->where("type !=","category_second")
            ->where("type !=","category_first")->where("type !=","system")->where("type !=","result")->where("part_id",$part_id)->order_by("sort","asc")->get("lessons_part_events")->result();
        $result = array();
        $result['events'] = array();
        $i = 1;
        foreach($evnets as $key => $event){
            ...
            if($eventItem->id != 0){
                ...
                $data = array(
                    ...
                    "avatar"  =>  $avatarname,
                    "role"               => $eventItem->name,
                    "category_first_id"=>$eventItem->category_first_id,
                    "category_second_id"=>$eventItem->category_second_id
                );
    ...
    public function getJsonAllPartEventListforMt($unit,$types,$part){
        ...
        if(count($lessons) >0){
            ...
            if(count($parts) >0){
                $evnets = $this->db->select('id')->where("type !=","system")->where("type !=","category_second")
                    ->where("type !=","category_first")
                    ->where("type !=","result")
                    ->where_in("part_id",$this->toIdArray($parts))
                    ->where_in("type",$types)->order_by("sort","asc")->get("lessons_part_events")->result();
            }else{
                return $result;
            }
    ...
    public function getJsonAllPartEventList($lesson,$part){
        // $lessons = $this->db->select('id')->where("type !=","system")->where("type !=","result")->where("lesson_id",$lesson->id)->order_by("sort","asc")->get("lessons_part_events")->result();
        $result = array();
        $result['events'] = array();
        $i = 1;
        ...
        if(count($parts) >0){
            $evnets = $this->db->select('id')  ->where("type !=","category_second")
                ...
                ->where("type !=","system")
                ->where_in("part_id",$this->toIdArray($parts))
                ->order_by("sort","asc")->get("lessons_part_events")->result();
        }else{
            return $result;
        }
...
//                        $data['media_type']     = $file->type;
    ...
    public function getEventObject($eventItem){
        $event = new self;
        $event->id = $eventItem->id;
        $event->unit_id = $eventItem->unit_id;
        $event->content_id = $eventItem->content_id;
        $event->part_id = $eventItem->part_id;
        $event->type = $eventItem->type;
        $event->media_filename = $eventItem->media_filename;
        $event->media_type = $eventItem->media_type;
        $event->lesson_id = $eventItem->lesson_id;
        $event->timeRange = $eventItem->timeRange;
        $event->text = $eventItem->text;
        $event->content = $eventItem->content;
        $event->answer = $eventItem->answer;
        $event->answers = $eventItem->answers;
        $event->picture = $eventItem->picture;
        $event->pictures = $eventItem->pictures;
        $event->displayKewords = $eventItem->displayKewords;
        $event->scores = $eventItem->scores;
        $event->media_file_ids = $eventItem->media_file_ids;
        $event->picture_file_ids = $eventItem->picture_file_ids;
        $event->avatar_id = $eventItem->avatar_id;
        $event->left_avatar_id = $eventItem->left_avatar_id;
        $event->name = $eventItem->name;
        $event->user_id = $eventItem->user_id;

        $event->modify_user_id = $eventItem->modify_user_id;

        $event->create_time = $eventItem->create_time;
        $event->modify_time = $eventItem->modify_time;
        $event->category_first_id = $eventItem->category_first_id;
        $event->category_second_id = $eventItem->category_second_id;
    ...
    public function getEventList($part_id=0,$limit=20,$start=0){
        $evnets = $this->db->select('*')->where("part_id",$part_id)->order_by("sort","asc")->limit($limit,$start)->get("lessons_part_events")->result();
        $result = array();
        foreach($evnets as $evnet){
            // $result[] = $this->getEvent($evnet->id)->getEventInfo();
            $result[] = $this->getEventObject($evnet)->getEventInfo();
        }
    ...
    public function getEventTypes($course){
        if($course->type == "primary_school"){
            $childType = 1;
        }else if($course->type == "middle_school"){
            $childType = 2;
        }else{
            $childType = 3;
        }
        $eventtypes  = $this->db->select("*")
            ->where("( course_type = '".$course->type."' OR  sub_course_type = '".$course->type."'  OR  is_all = 1 OR childType= {$childType})")
    ...
    public function save($post){
        ...
        if(isset($data->id) && $data->id != 0){
            ...
            if(isset($data->category_first_id)){
                $entity['category_first_id'] =$data->category_first_id;
            }

            if(isset($data->category_second_id)){
                $entity['category_second_id'] =$data->category_second_id;
            }
            ...
            $entity = array(
                ...
                "category_first_id"=> isset($data->category_first_id)?$data->category_first_id:0,
                "category_second_id"=>isset($data->category_second_id)?$data->category_second_id:0,
...
//                "score"=>$data->score,
            ...
            if((isset($data->itemEntity))){
                $content_id = $data->itemEntity->content_id!=0?$data->itemEntity->content_id:$content_id;
                $contentEntity  = array(
                    "file_id"   => $data->media_file_ids,
                    "text"      => $data->text,
                    "timeRange" => $data->timeRange,
...
//                "score"=>$data->score,
            ...
            if((isset($data->clickContent->content_id)) ){
                $content_id = $data->clickContent->content_id!=0?$data->clickContent->content_id:$content_id;
...
//                "score"=>$data->score,
    ...
    public function removeAllEvents($part){
        $evnets = $this->db->select('id')->where("part_id",$part->id)->get("lessons_part_events")->result();
        foreach($evnets as $evnet){
    ...
    public function getJsonFileAllMenuList($lesson,$part){
        ...
        $data = array();
        $evnet = $this->db->select('*')
            ->where("type !=","category_second")
            ->where("type !=","category_first")
            ->where("type !=","system")
            ->where("type !=","result")->where("part_id",$part->id)->get("lessons_part_events")->row();
    ...
    public function getJsoncategoryFirstList($part){
        $evnets = $this->db->select('*')
            ->where("type ","category_first")
            ->where("part_id",$part->id)->get("lessons_part_events")->result();
        foreach($evnets as $key=>$evnet){
            if(!empty($evnet->picture_file_ids)){
                $file = $this->wetalkfile->getFile((int)$evnet->picture_file_ids);
                $filename = 'media/'.$file->filename;
                $data[]= array(
                    "num"                => $key+1,
                    'type'               => $evnet->type,
                    'id'                 => $evnet->id,
                    "text"               => (string)$evnet->text,
                    "key"               => (string)$evnet->picture_file_ids,
                    "media_filename"     => $filename,
                    "media_type"         => 'image',
                    "picture"            =>  "",
                );
    ...
    public function getJsoncategorySecondList($part){
        $evnets = $this->db->select('*')
            ->where("type ","category_second")
            ->where("part_id",$part->id)->get("lessons_part_events")->result();
        foreach($evnets as $key=>$evnet){
    ...
    //同步所有内容
    public function syncAllEvents($part,$orgin_part_id){
        $orginEvents = $this->db->where("part_id",$orgin_part_id)->get("lessons_part_events")->result_array();
        foreach($orginEvents as $orginEvent){
            $orginEvent['part_id']  = $part->id;
            $orgin_event_id = $orginEvent['id'];
    ...
    public function getEventFirstCategory($part){
        $categorys = array();
        if(isset($part->id)){
    ...
    public function getEventSecondCategory($part){
        $categorys = array();
        if(isset($part->id)){
            $categorys  = $this->db->select("*")
                ->where("type","category_second")
