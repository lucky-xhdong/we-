...
//            echo $this->db->last_query();
    ...
    public function getUnitList($user,$publishing_house_course_id=1,$publishing_house_course_level_id=0,$flag=false){
        $get = $this->input->get();
        if(!isset($get['app_version'])) $get['app_version'] = "1.0.2";
        ...
        if($get['app_version'] < "1.2.0" && $flag){
            $units =  $units->where("units.id <",123);
        }
        $units = $units->where("publishing_house_course_id",$publishing_house_course_id)
    ...
    public function getkUnitsLearningTimeInfo($course_id=0,$user=null,$date){
        $this->recordmanager->initialize()->user_id = $user->id;
        $unitObJ =  $this->recordmanager->getMyDateStudyUnitsTimeData($date);
        if($user->id == 53){
            $units = $this->db->select("levels.name as levelName,units.*,units.name as unitnewname,units.id as publishing_house_course_unit_id")
                ->join('levels','levels.id = units.level_id', 'left')
                ->join('courses','units.course_id = courses.id', 'left')
                ->where("courses.id",$course_id)
                ->where("levels.id IS NOT NULL")
                ->where_in("units.id",$this->toIdArray($unitObJ))
                ->get('units')->result();
        }else{
            $units = $this->db->select('publishing_house_course_units.unit_id,publishing_house_course_units.id as publishing_house_course_unit_id ,publishing_house_course_units.name as unitnewname,units.*,publishing_house_course_levels.name as levelName')
                ->join('units','units.id = publishing_house_course_units.unit_id', 'left')
        ...
        foreach($units as $unit){
            ...
            $unitItem['name'] =  $unit->levelName.'-'.$unitItem['name'];
            $unitItem['lessons'] = $this->lesson->getLessonLearningTimeList($user,$unit->id,$date)->returncode['data'];
            $result[] = $unitItem;
        }
        return $result;
    }
    ...
    public function getpublichUnitsMtInfo($course_id=0,$user=null){
        ...
        foreach($units as $unit){
            $unitItem = $this->getUnitInfo1($user,$unit);
            if(($get['app_version'] < "1.2.0" && $get['device_platform'] == "iphone") &&  $unit->levelName != "we talk"){
                $unitItem['name'] =  $unitItem['name'];
            }else if($unit->levelName != "we talk"){
                $unitItem['name'] =  $unit->levelName.'-'.$unitItem['name'];
            }
            //$unitItem['name'] = $unitItem['name'];
            //$unitItem['mt_score'] = 0;
            $lesson = $this->db->select('id')->where("type","test")->where("unit_id",$unit->unit_id)->get("lessons")->row();
            if($get['device_platform'] == "pc" ){
                if($lesson && $user){
                ...
                //   $unitItem['mt_score'] = (int)$auhtUnit->mt_score;
//                if($lesson && $user){
//                    $auhtUnit= $this->courseauth->getUserLessonRowChain($user,(int)$lesson->id);
//                    if($auhtUnit){
//
//                    }
//                }
