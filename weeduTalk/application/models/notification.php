...
//				$this->returncode['errdesc'] = "你已经关注了他";
	...
	private function getNotificationText($usera=null,$userb=null){
		$body = "";
		if($this->_minxer) {
...
//							}
				...
				if($this->_minxer->type == "group"){
					...
					if($usera) $mixer = $usera;
					else  $mixer = $this->user->getUser($this->a_id);
					$word= "你";
				}
...
//							}
	...
	public function getNotificationList($limit=20,$start=0){
		...
		$notifications = $this->db
			->select(implode(',',array_keys($entity)))
			->where('b_id',$this->_minxer->id)
			->where("status",0)
			->order_by("id","desc")->limit($limit,$start)
			->get("notifications")->result();
		foreach($notifications as $item1){
			$user_array_ids[] = $item1->a_id;
		}
		$users = array();
		...
		foreach($notifications as $notification){
			...
			$userItem = $users[$notification->a_id];
			if(empty($userItem->id)) continue;
 				$data[] = array(
					...
					"type"			=>  $notification->type,
					"userInfo"		=>  $userItem,
					"body"			=>  $this->init($notification->id)->setMinxer($this->_minxer)->getNotificationText($userItem),
				);
		}
	...
	public function getNotificationUnreadCount(){
		return 0;
		$notification = $this->db->where('unread',1)->where('b_id',$this->_minxer->id)->where("status",0)->get("notifications")->num_rows();
		return $notification;
	}
