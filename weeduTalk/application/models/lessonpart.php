...
//        }
        ...
        $data['CategoryFirsts'] = $this->event->getJsoncategoryFirstList($this);
        $data['CategorySeconds'] = $this->event->getJsoncategorySecondList($this);
...
//                        "title"=>"语音识别的回答问题:看图片内容和选项,根据听到的问题,读出正确的选项;"),
    ...
    public function getLessonpartListNew($user,$lesson_id=0,$flag=0){
        $post = $this->input->get();
        //$lesson = $this->db->where("id",(int)$lesson_id)->get("lessons")->row();
        if(($user && $user->id == 53) || $flag == 1){

            $lessons = $this->db->select('*')->where("lesson_id",(int)$lesson_id)->order_by("sort","asc")->get("lessons_part")->result();
            //echo $this->db->last_query();exit;
        }else{

            $lessons = $this->db->select('*')->where("status",1)
                ->where("lesson_id",(int)$lesson_id);
            $lessons = $lessons->order_by("sort","asc")
                ->get("lessons_part")->result();
        }
        ...
        foreach($lessons as $lesson){
            ...
            $result[]  = array(
                ...
                'type'            => $lesson->type,
                "tips"            => $lesson->tips,
                "tag_type"        => $lesson->tag_type,
                "tag_name"        => $lesson->tag_name,
    ...
    public function getSpeechContentList($user,$limit=20,$start=0){
        ...
        if($user->user_type == "teacher"){
            ...
            if($class_user_relationships){
                $recordItems = $this->db
                    ->select("users_record_speech.*,class_user_relationship.class_id,classes.name as ClassName,grades.name as GradeName,users.name as Username")
                    ...
                    ->where("users_record_speech.type","speech")
                    ->where("users_record_speech.filename is not null")
                    ->where_in("users_record_speech.part_id",$this->toIdArray($event_part_id))
                    ->join('users_record_speech','class_user_relationship.user_id = users_record_speech.user_id', 'left')
                    ->join('classes', 'classes.id = class_user_relationship.class_id', 'left')
                    ->join('grades', 'classes.grade_id = grades.id', 'left')
                    ->join('users', 'users.id = users_record_speech.user_id', 'left')
                ;

                if($is_read_papers !== false){
                    if($is_read_papers == 1){
                        $recordItems = $recordItems
                            ->where("users_record_speech.comment_text_status",1)
                            ->where("users_record_speech.comment_speech_status",1);
                    }else{
                        $recordItems = $recordItems
                            ->where("users_record_speech.comment_text_status",0)
                            ->where("users_record_speech.comment_speech_status",0);
                    }
                }

                if($time_sort !== false ){
                    if($time_sort == 1){
                        $recordItems = $recordItems->order_by("users_record_speech.start_time","desc");
                    }else{
                        $recordItems = $recordItems->order_by("users_record_speech.start_time","asc");
                    }
                }else{
                    $recordItems = $recordItems->order_by("users_record_speech.start_time","desc");
...
//                    else $record->gradeName = "";
            ...
            $recordItems = $this->db
                ->select("users_record_speech.*,class_user_relationship.class_id,users.name as Username,classes.name as ClassName,grades.name as GradeName")
                ->where("users_record_speech.type","speech")
                ->where("users_record_speech.filename is not null")
                ->where_in("users_record_speech.part_id",$this->toIdArray($event_part_id))
                ->join('class_user_relationship','class_user_relationship.user_id = users_record_speech.user_id', 'left')
                ->join('classes', 'classes.id = class_user_relationship.class_id', 'left')
                ->join('grades', 'classes.grade_id = grades.id', 'left')
                ->join('users', 'users.id = users_record_speech.user_id', 'left')

                ->where("users_record_speech.user_id",$user->id);
            if($time_sort !== false){
                if($time_sort == 1){
                    $recordItems = $recordItems->order_by("users_record_speech.start_time","desc");
                }else{
            ...
            $recordItems = $recordItems->limit($limit,$start)->get('users_record_speech')->result();
            foreach($recordItems as $recordItem){
                $record = $this->getRecordItem($recordItem,$user);
                $record->gradeName = "";
                $records[] = $record;
...
//            if( $record->id >= 170029699){
//                $record->objectType = "speechNew";
//            }else{
//
//            }

            $realfiepath = UPLOADFILEPATH.$record->objectType.'/n'.$record->id.'/' . $record->filename;
            if(!is_file($realfiepath)){
                $record->objectType = "speechNew";
            }
...
//            else $record->gradeName = "";
    ...
    public function getRecordDetail($user,$record_id=0){
        ...
        $record1 = $this->db
            ->select("users_record_speech.*,class_user_relationship.class_id,users.name as Username,classes.name as ClassName,grades.name as GradeName")
            ->where("users_record_speech.id",$record_id)
            ->join('class_user_relationship','class_user_relationship.user_id = users_record_speech.user_id', 'left')
            ->join('classes', 'classes.id = class_user_relationship.class_id', 'left')
            ->join('grades', 'classes.grade_id = grades.id', 'left')
            ->join('users', 'users.id = users_record_speech.user_id', 'left')
            ->get('users_record_speech')->row();
