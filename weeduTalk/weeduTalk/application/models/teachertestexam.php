...
class Teachertestexam extends CI_Model{
    ...
    public function getTeachertestexamRow($user=null,$data= array()){
        $result = array();
        $teacher_test_exam = $this->db->select('*')->order_by("sort","asc")->get("teacher_test_exam")->row();
    ...
    public function getTeachertestexamList($user=null){
        $teacher_test_exam = $this->db->select('*')->where("status",1)->order_by("sort","asc")->get("teacher_test_exam")->result();
