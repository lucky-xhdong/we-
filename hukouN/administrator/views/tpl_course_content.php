    ...
    <main class="main-carema-list">
        <div class="breadcrumbs">
            <ul>
                ...
                <li>
                    <a <?php if ($controller == 'course' && $action == 'content') echo 'class="active"'; else echo 'class=""'; ?> href="javascript:;">内容管理</a>
                </li>
            </ul>
        </div>
        <div class="setrow setting-right-bg bg-color-fff">
            <div class="org-courseoutline-main">
                <div class="complex-select">
                    <ul>
                        <li>
                            <a class="btn btn-success btn-new-firstdir btn-new-first-course-dir add-dir" href="javascript:;" data-level="0">新建一级栏目</a>
                        </li>
                    </ul>
                </div>
                ...
                <div id="course-outline-new-activity" class="courseoutline-newactivity">
                    <!--                        <iframe src="" id="story-iframe" name="story-iframe" class="width-100 border-none">-->
                    <!--                        </iframe>-->
                </div>
            </div>
        </div>
...
<div id="course-outline-first-dirtmpl" type="text/x-jquery-tmpl" style="display:none">
    <li>
        <div class="courseoutline-first-dirlist" data-id="${id}">
            ...
            <div class="org-outline-handle">
<!--                --><?//= anchor('course/contentmanage/${id}','内容管理', 'data-psign="${psign}" data-upid="${upid}"
//                   data-level="${level}" data-sign="${sign}" data-id="${id}" target="_blank"')?>
<!--                <hgroup class="activity-group">-->
<!--                    --><?//= anchor('course/publishcontent/${id}','发内容', 'class="courseoutline-newactivity" data-psign="${psign}" data-upid="${upid}"
//                       data-level="${level}" data-sign="${sign}" data-id="${id}" data-url=""')?>
<!--                </hgroup>-->
                <a href="javascript:;" class="add-dir" data-psign="${psign}" data-upid='${upid}' data-level='${level}'
                   data-sign="${sign}" data-id="${id}"><i class="fa fa-plus"></i></a>
                <a href="javascript:;" class="editcourse" data-psign="${psign}" data-upid='${upid}'
                   data-level='${level}' data-sign="${sign}" data-id="${id}"><i class="fa fa-edit"></i></a>
                <a href="javascript:;" class="deletecourse" data-psign="${psign}" data-upid='${upid}'
                   data-level='${level}' data-sign="${sign}" data-id="${id}"><i class="fa fa-times"></i></a>
            </div>
        </div>
        <ul id="coursedownunits${id}">
...
<div id="course-outline-second-dirtmpl" type="text/x-jquery-tmpl" style="display:none">
    <li>
        <div class="courseoutline-second-dirlist" data-id="${id}">
            ...
            <div class="org-outline-handle">
                <?= anchor('course/contentmanage/${id}/${courseid}','内容管理', 'data-psign="${psign}" data-upid="${upid}"
                   data-level="${level}" data-sign="${sign}" data-id="${id}" target="_blank"')?>
                <hgroup class="activity-group">
                    <?= anchor('course/publishcontent/${id}','发内容', 'class="courseoutline-newactivity" data-psign="${psign}" data-upid="${upid}"
                       data-level="${level}" data-sign="${sign}" data-id="${id}" data-url=""')?>
                ...
                <a href="javascript:;" class="add-dir" data-psign="${psign}" data-upid='${upid}' data-level='${level}'
                   data-sign="${sign}" data-id="${id}"><i style="width: 11px;display: block;"></i></a>
                <a href="javascript:;" class="editcourse" data-psign="${psign}" data-upid='${upid}'
                   data-level='${level}' data-sign="${sign}" data-id="${id}"><i class="fa fa-edit"></i></a>
                <a href="javascript:;" class="deletecourse" data-psign="${psign}" data-upid='${upid}'
                   data-level='${level}' data-sign="${sign}" data-id="${id}"><i class="fa fa-times"></i></a>
        ...
        <ul>
            ...
            <li>
                <div class="org-outline-subcaption">
                    <a href="<?=anchorurl('course/publishcontent/')?>/${syllabus_id}/${id}" title="${title}">${title}</a>
                    {{if time}}
        ...
        <div class="dis-table-cell course-outline-new-dir-options">
            <form action="">
                <div class="input-group">
                    ...
                    <input type="text" placeholder="栏目名称" value="${name}" name="name" class="form-control input-course-name color-f9c600" id="input-course-name">
                ...
                <div class="btn-course-group">
                    <button class="btn btn-success float-left fangcaodi-btn-style" type="button" id="save-first-dir">确定</button>
...
//                                    document.getElementById('course-outline-first-dir').unspin();
        ...
        function getcourseoutline() {
            $.ajax({
                url: '<?=anchorurl('course/getcourseoutline')?>/'+courseid,
                type: "get",
                dataType: "json",
                success: function (data) {
                    units.length = 0;
