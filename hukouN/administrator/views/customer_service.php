  ...
  <div class="row"> 
    ...
    <div class="col-sm-12 col-md-12 h-o">
      <div class="col-sm-12 col-md-4">
      	<div class="row">
          <div class="fl bg-color-e pad-h-m  li-ul-pad li-ul-mar li-col-12 li-current-consult">
              <ul id="user_ul">
      ...
      <div class="col-sm-12 col-md-8">
      	<div class="row">
          <div class="h-o pt-re peoplechat" id="chatSection">
        ...
        <div class="pt-re">
          <div class="pad-h-m dis-d input-chat-section">
            <textarea class="form-control bor-ra-a border-none font-size-12" placeholder="输入文字" name="msgcontent" id="msgcontent"></textarea>
          </div>
          <div class="h-o pad-h-m">
            <div class="fl btn-add-chat">
              <a href="javascript:void(0)" class="btn-add-text btn-add-text-in" >
                <i class="pad-h-a fa font-s-lg fa-font color-b"></i>
              </a>
              <a href="javascript:void(0)" class="btn-add-img" id="inserimage">
                <i class="pad-h-a fa font-s-lg fa-picture-o color-b" ></i>
              </a>
            </div>
            <button class="fr btn btn-success bor-ra-a" id="sendmessage">发送</button>
          </div>
        </div>
        <div class="clear-13"></div>
      </div>
...
<div class="clear-13"></div>
 ...
 <div id="receiveChatTemplate" type="text/x-jquery-tmpl" style="display:none">
  ...
  <div id="sendChatTemplate" type="text/x-jquery-tmpl" style="display:none">
    ...
    <div id="questionTemplate" type="text/x-jquery-tmpl" style="display:none">
    <li data-id='${question_id}' data-src='${user_id}' data-name='${user_name}'>
          <div class="dis-d pad-e chat-first-menu">
              <a class="d-table-cell fl pad-r-a pt-re chat-sm-img" href="javascript:void(0)">
                <img src="${header_picture_url}"> 
              </a>
              <div class="d-table-cell ver-a-b chat-person-info">
                ...
                <span class="badge dis-f pull-right" id="unread_${question_id}"></span>
                ${user_name}
                </a>
              </div>
          </div>
...
<!--footer start-->
<div class="text-b container">
    <span class="dis-a pad-h-m">&copy;2014-9-1;</span>
</div>
<!--footer end-->                               
<script>
	var question_id = 0;
	...
	$(document).ready(function(e) {
		...
		function sendMsg(){
			$.ajax({
				  type :'POST',
				  url :'<?=anchorurl('chat/send_client_message')?>',
				  data:{
				  	 'dst_user_id':src_user_id,
				  ...
				  success: function(data){
					  ...
					  if(typeof data.result != 'undefined' && !data.result){
					   ...
					   var currentMsgTime = dateFormat(new Date((data.create_time)*1000), 'yyyy年MM月dd日 hh:mm');
							data.time = currentMsgTime; 
							last_msgid = data.msg_id;
						 $("#msgcontent").val(''); 
						 $('#chatSection').hideLoading();
						$('#sendChatTemplate').tmpl(data).appendTo('#user_message');
						$(".chatsection").scrollTop($(".chatsection").height()+$(".chatsection")[0].scrollHeight);
					  }
				  }
				});
		...
		function sendmessage(){
			$("#sendmessage").unbind('click').bind('click',function(){
				sendMsg();
			});
		}
		
		$('#msgcontent').off('keydown').on('keydown', function (event) {
        if (event.keyCode == "13") {
          sendMsg();
          return false;
        }
	   ...
	   function get_recent_message(){
	   	 $.ajax({
			  type :'POST',
			  url :'<?=anchorurl('chat/get_recent_message')?>',
			  dataType:"json",
			  data:{
				'status':<?=$status?>  
			  },
			  success: function(data){
				 ...
				 $(data.question).each(function(index, element) {
						...
						$('#questionTemplate').tmpl(element).appendTo('#user_ul');
					
					// 有新消息
					if(element.unreadmessage >0){
						$("#unread_"+element.question_id).removeClass("dis-f").html(element.unreadmessage);
					}
                });
				$("#user_ul li").removeClass('bg-color-h');
				$("#unread_"+question_id).addClass("dis-f");
				$("#user_ul li[data-id='"+question_id+"']").addClass('bg-color-h');
			  }
			}); 
	   }
	   get_recent_message();
	   var time = setInterval(get_recent_message,10*1000)
		...
		function onMSG(){
			if(question_id){
				$.ajax({
					  type :'GET',
					  url :'<?=anchorurl('chat/get_user_message')?>'+'/'+question_id+'/'+last_msgid,
					  dataType:"json",
					  success: function(data){
						 $(data.msg).each(function(index, element) {
								var currentMsgTime = dateFormat(new Date((element.create_time)*1000), 'yyyy年MM月dd日 hh:mm');
		 ...
		 var time1 = setInterval(onMSG,5*1000)
    });
